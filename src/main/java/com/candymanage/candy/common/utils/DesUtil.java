...
public class DesUtil {
     ...
     */
    public static String dencode(String data, String firstKey, String secondKey, String thirdKey) {
        int leng = data.length();
        String decStr = "";
        List firstKeyBt = null, secondKeyBt = null, thirdKeyBt = null;
        int firstLength = 0, secondLength = 0, thirdLength = 0;
    ...
    private static List getKeyBytes(String key) {
        ...
        if (remainder > 0) {
            ...
            if (iterator == 0) {
                keyBytes.add(0, strToBt(key.substring(0 * 4 + 0, leng)));
            } else {
                keyBytes.add((iterator - 1), strToBt(key.substring((iterator - 1) * 4 + 0, leng)));
            }
        }
        return keyBytes;
    }
    ...
    private static String bt64ToHex(int[] byteData) {
        ...
        for (int i = 0; i < 16; i++) {
            String bt = "";
            for (int j = 0; j < 4; j++) {
                bt += String.valueOf(byteData[i * 4 + j]);
            }
            hex += bt4ToHex(bt);
        }
        return hex;
